...
const resolvers = {
  Query: {
    allBooks: (root, args) => {
      const byAuthor = args.author ? (booksByAuthor[args.author] || []) : books
  ...
  Mutation: {
    addBook: (root, args) => {
      ...
      booksByAuthor[book.author] = [...booksByAuthor[book.author] || [], book]

      if (!authors.map(a => a.name).includes(book.author)) {
        const author = { id: uuid(), name: book.author }
